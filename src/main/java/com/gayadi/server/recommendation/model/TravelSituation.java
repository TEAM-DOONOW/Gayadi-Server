package com.gayadi.server.recommendation.model;
...
public record TravelSituation(
    ...
    public Policy policy() {
        ...
        if (indoorRequired) {
            summary.append("실내 장소 우선");
        }
        else if (avoidOutdoor) summary.append("야외 활동 축소");
        if (avoidCrowded) {
            append(summary, "혼잡 장소 회피");
        }
        if (transitDisrupted) {
            append(summary, "대중교통 대체 경로 필요");
        }
        if (summary.isEmpty()) {
            summary.append("특이 상황 없음");
        }
