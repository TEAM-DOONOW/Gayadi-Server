...
import com.gayadi.server.weather.dto.response.ForecastVersionItemResponse;
...
/** 기상청 API 호출 결과를 실황과 예보 응답 DTO로 변환합니다. */
@Service
public class WeatherApiService {
    ...
    /** 초단기실황 조회 — 현재 날씨 관측값. */
    public UltraShortNowcastResponse ultraSrtNcst(WeatherRequest req) {
    ...
    public WeatherForecastResponse ultraSrtFcst(WeatherRequest req) {
        return forecast(OP_ULTRA_SRT_FCST, req,
                KmaBaseTimeCalculator.ForecastType.ULTRA_FCST, true);
    }

    /** 단기예보 조회 — 3~5일 기간 예보. */
    public WeatherForecastResponse vilageFcst(WeatherRequest req) {
        return forecast(OP_VILAGE_FCST, req,
                KmaBaseTimeCalculator.ForecastType.VILAGE_FCST, false);
    }

    /** 예보버전 조회. */
    public ForecastVersionResponse fcstVersion(ForecastVersionRequest req) {
        requireParam("ftype", req.fileType());
        requireParam("baseDateTime", req.baseDateTime());
        String fileType = req.fileType().trim().toUpperCase(java.util.Locale.ROOT);
        ...
        List<ForecastVersionItemResponse> versions = new ArrayList<>();
        for (JsonNode item : client.allItems(OP_FCST_VERSION, params)) {
            versions.add(new ForecastVersionItemResponse(
                    WeatherApiClient.text(item, "filetype"),
                    WeatherApiClient.text(item, "version")));
        }
        return new ForecastVersionResponse(versions);
    }
    ...
    private WeatherForecastResponse forecast(
        ...
        List<WeatherForecastSlotResponse> forecast = toSlots(client.allItems(operation, params), ultra);
        return new WeatherForecastResponse(
                dt.date(), dt.time(), grid[0], grid[1], forecast);
    }

    private List<WeatherForecastSlotResponse> toSlots(List<JsonNode> items, boolean ultra) {
        ...
        List<WeatherForecastSlotResponse> forecast = new ArrayList<>();
        for (Map.Entry<String, Map<String, String>> entry : bySlot.entrySet()) {
            String slot = entry.getKey();
            Map<String, String> v = entry.getValue();
            forecast.add(new WeatherForecastSlotResponse(
                    slot.substring(0, 8),
                    slot.substring(8),
